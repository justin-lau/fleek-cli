...
vi.mock('./prompts/getDomainOrPrompt', () => ({
  getDomainOrPrompt: vi.fn().mockResolvedValue({
    ...
    dnsConfigs: [{ type: 'CNAME', name: 'hostname', value: 'clee640yh0000l808a1nzrogc.b-cdn.net' }],
  }),
}));

describe('Show domain detailed information', () => {
  it('Find by hostname', async () => {
    await expect(detailDomainAction({ sdk: {} as FleekSdk, args: { hostname: 'first.xyz' } })).resolves.toBeUndefined();
    ...
    expect(fakeOutput.table).toHaveBeenNthCalledWith(1, [
      { Hostname: 'first.xyz', Status: 'ACTIVE', 'Created At': '2023-02-01T00:00:00.000Z' },
    ]);
    expect(fakeOutput.table).toHaveBeenNthCalledWith(2, [
      { Type: 'CNAME', Name: 'hostname', Value: 'clee640yh0000l808a1nzrogc.b-cdn.net' },
    ]);
  });
});
