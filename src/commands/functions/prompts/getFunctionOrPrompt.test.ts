...
vi.mock('@fleek-platform/sdk', () => {
  ...
  const functions = {
    get: vi.fn().mockResolvedValue({ name: 'firstFunctionName', slug: 'first-first-first', id: 'firstFunctionId' }),
    getBySlug: vi.fn().mockResolvedValue({ name: 'secondFunctionName', slug: 'second-second-second', id: 'secondFunctionId' }),
    list: vi.fn().mockResolvedValue([
      { name: 'firstFunctionName', slug: 'first-first-first', id: 'firstFunctionId' },
      { name: 'secondFunctionName', slug: 'second-second-second', id: 'secondFunctionId' },
    ]),
  };
...
describe('Get function by name, or let the user choose from list', () => {
  it('Return function by its name', async () => {
    ...
    await expect(getFunctionOrPrompt({ sdk: fakeSdk, name: 'firstFunctionName' })).resolves.toEqual({
      name: 'firstFunctionName',
      slug: 'first-first-first',
      id: 'firstFunctionId',
    });

    expect(fakeSdk.functions().get).toHaveBeenCalledWith({ name: 'firstFunctionName' });
  });
  ...
  it('should throw if no functions exist', async () => {
    ...
    await expect(getFunctionOrPrompt({ sdk: fakeSdk })).rejects.toThrowError(new FleekFunctionsNotFoundError({}));
