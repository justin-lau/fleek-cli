...
describe('Get function code', () => {
  it('returns the function code', async () => {
    await expect(getFunctionPathOrPrompt({ path: __dirname + '/testNoPrompt.js' })).resolves.toStrictEqual(__dirname + '/testNoPrompt.js');
  });
});

describe('Prompt user for function code path', () => {
  it('shows text prompt for function code', async () => {
    await expect(getFunctionPathOrPrompt({})).resolves.toStrictEqual(__dirname + '/test.js');
