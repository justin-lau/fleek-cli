...
vi.mock('@fleek-platform/sdk', () => {
  ...
  const privateGateways = {
    list: vi.fn().mockResolvedValue([
      { id: 'firstPrivateGatewayId', slug: 'one-blue-fish', name: 'first', createdAt: '2023-02-01T00:00:00.000Z' },
      { id: 'secondPrivateGatewayId', slug: 'two-green-fix', name: 'second', createdAt: '2023-02-02T00:00:00.000Z' },
    ]),
  };
...
describe('List all private gateways', () => {
  it('List 2 private gateways assigned to selected project', async () => {
    ...
    await expect(listPrivateGatewaysAction({ sdk: fakeSdk, args: {} })).resolves.toBeUndefined();
    ...
    expect(output.table).toHaveBeenCalledWith([
      ...
      { ID: 'secondPrivateGatewayId', Slug: 'two-green-fix', Name: 'second', 'Created At': '2023-02-02T00:00:00.000Z' },
    ]);
  });
  ...
  it('Show message that no private gateways exist', async () => {
    ...
    expect(output.log).toHaveBeenCalledWith('You currently do not have any private gateways configured.');
