...
vi.mock('prompts', () => ({ default: vi.fn().mockResolvedValue({ value: 'second' }) }));

describe('Validate private gateway name or let the user enter name', () => {
  it('Return valid name', async () => {
    await expect(getPrivateGatewayNameOrPrompt({ name: 'first' })).resolves.toEqual('first');

    expect(prompts).not.toHaveBeenCalled();
  });

  it('Throw an error on invalid name', async () => {
    await expect(getPrivateGatewayNameOrPrompt({ name: '*' })).rejects.toEqual(new PrivateGatewayNameInvalidError({ name: '*' }));
