...
vi.mock('@fleek-platform/sdk', () => {
  ...
  const privateGateways = {
    get: vi.fn().mockResolvedValue({ id: 'thirdPrivateGatewayId', slug: 'three-white-apple', name: 'third gateway' }),
    getBySlug: vi.fn().mockResolvedValue({ id: 'firstPrivateGatewayId', slug: 'one-green-fish', name: 'first gateway' }),
    list: vi.fn().mockResolvedValue([
      { id: 'firstPrivateGatewayId', slug: 'one-green-fish', name: 'first gateway' },
      { id: 'secondPrivateGatewayId', slug: 'two-blue-fox', name: 'second gateway' },
      { id: 'thirdPrivateGatewayId', slug: 'three-white-apple', name: 'third gateway' },
    ]),
  };
...
describe('Get private gateway by id or slug. When nothing passed let the user choose from list', () => {
  ...
  it<TestContext>('Return private gateway by its id', async (context) => {
    await expect(getPrivateGatewayOrPrompt({ sdk: context.fakeSdk, id: 'thirdPrivateGatewayId' })).resolves.toEqual({
      id: 'thirdPrivateGatewayId',
      slug: 'three-white-apple',
      name: 'third gateway',
    });

    expect(context.fakeSdk.privateGateways().get).toHaveBeenCalledWith({ id: 'thirdPrivateGatewayId' });
  ...
  it<TestContext>('Return private gateway by its slug', async (context) => {
    await expect(getPrivateGatewayOrPrompt({ sdk: context.fakeSdk, slug: 'one-green-fish' })).resolves.toEqual({
    ...
    expect(context.fakeSdk.privateGateways().getBySlug).toHaveBeenCalledWith({ slug: 'one-green-fish' });
    expect(context.fakeSdk.privateGateways().list).not.toHaveBeenCalled();
    expect(prompts).not.toHaveBeenCalled();
  });

  it<TestContext>('Let the user choose from private gateways and return chosen private gateway', async (context) => {
    await expect(getPrivateGatewayOrPrompt({ sdk: context.fakeSdk })).resolves.toEqual({
  ...
  it<TestContext>('Throw an error because no private gateways available under current project', async (context) => {
    ...
    await expect(getPrivateGatewayOrPrompt({ sdk: context.fakeSdk })).rejects.toEqual(new PrivateGatewaysNotFoundError({}));
