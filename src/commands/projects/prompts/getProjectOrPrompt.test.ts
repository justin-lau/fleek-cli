...
vi.mock('../../../prompts/selectPrompt', () => ({ selectPrompt: vi.fn().mockResolvedValue('secondProjectId') }));

vi.mock('@fleek-platform/sdk', () => {
  const FleekSdkMock = vi.fn();

  const projects = {
    get: vi.fn().mockResolvedValue({ id: 'firstProjectId', name: 'first project' }),
...
describe('Get project by its id or let the user choose from list', () => {
  it('Return project by its id', async () => {
    ...
    await expect(getProjectOrPrompt({ sdk: fakeSdk, id: 'firstProjectId' })).resolves.toEqual({
      id: 'firstProjectId',
      name: 'first project',
    });

    expect(fakeSdk.projects().get).toHaveBeenCalledWith({ id: 'firstProjectId' });
  ...
  it('Throw an error because of no projects exist', async () => {
    ...
    await expect(getProjectOrPrompt({ sdk: fakeSdk })).rejects.toThrowError(new ProjectsNotFoundError());
  ...
  it('Let the user choose from list of existing projects and return chosen project', async () => {
    ...
    await expect(getProjectOrPrompt({ sdk: fakeSdk })).resolves.toEqual({ id: 'secondProjectId', name: 'second project' });
