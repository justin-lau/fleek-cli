...
vi.mock('@fleek-platform/sdk', () => {
  ...
  const sites = {
    get: vi.fn().mockResolvedValue({ id: 'firstSiteId', slug: 'first-first-first' }),
    getBySlug: vi.fn().mockResolvedValue({ id: 'secondSiteId', slug: 'second-second-second' }),
...
describe('Get site by id, slug or let the user choose from list', () => {
  it('Return site by its id', async () => {
    ...
    await expect(getSiteOrPrompt({ sdk: fakeSdk, id: 'firstSiteId' })).resolves.toEqual({ id: 'firstSiteId', slug: 'first-first-first' });
  ...
  it('Return site by its slug', async () => {
    ...
    await expect(getSiteOrPrompt({ sdk: fakeSdk, slug: 'second-second-second' })).resolves.toEqual({
      id: 'secondSiteId',
      slug: 'second-second-second',
    });

    expect(fakeSdk.sites().getBySlug).toHaveBeenCalledWith({ slug: 'second-second-second' });
  });
  ...
  it('Let the user choose from list and return chosen site', async () => {
    ...
    await expect(getSiteOrPrompt({ sdk: fakeSdk })).resolves.toEqual({ id: 'secondSiteId', slug: 'second-second-second' });
  ...
  it('should throw if no sites exist', async () => {
    ...
    await expect(getSiteOrPrompt({ sdk: fakeSdk })).rejects.toThrowError(new SitesNotFoundError());
