...
vi.mock('@fleek-platform/sdk', () => {
  ...
  const storage = {
    delete: vi.fn((options: { cid: string }) => {
      if (options.cid === 'bafkreiebwzjtd62ctklmmidldy2z2exinzr2mc72tzhkbe7ftjxm7cwnle') {
    ...
    getByFilename: vi.fn((options: { filename: string; extension: string }) => {
      const batchProcessingData = [...Array(25).keys()].map((i) => ({ filename: 'basicBatch.car', cid: `bafyrandom${i}` }));
