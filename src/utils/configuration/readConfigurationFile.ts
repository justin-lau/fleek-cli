...
export const readConfigurationFile = async ({
...
}: ReadConfigurationFileArgs): Promise<{ configuration: object; configPath: string }> => {
  const configPath = await getConfigurationPath({ predefinedConfigPath });

  const fileExtension = getExtension(configPath);

  if (fileExtension === '.json') {
    const content = await fs.readFile(configPath, 'utf8').catch(() => Promise.reject(new FleekConfigMissingFileError({})));

    try {
      return { configuration: JSON.parse(content), configPath };
    } catch (e) {
      throw new FleekConfigInvalidContentError({ configPath, validationResult: t('jsonNotValid') });
    }
  }
  ...
  throw new FleekConfigInvalidContentError({ configPath, validationResult: t('unknownFileExt') });
};
